...
*How to model the real exposure surfaces — AI analyzers, tokenizers, semantic search, RAG, supply chain, and the post-usage blind spot — and why the LLM is itself a new kind of DLP.*

Most "AI security" diagrams are theater. They draw a box labeled *Guardrails* between the user and the model, slap a shield emoji on it, and call the risk closed. Then private data leaks anyway — through the vector database nobody classified as sensitive, through a tokenizer API nobody thought was an exposure surface, through a summarization feature that shipped your SIEM logs to a third party.
...
Almost nobody lists the tokenizer as a privacy surface. It should be near the top. Token, log-probability, and hidden-state APIs let an attacker invert text back out. Morris et al. (2023) demonstrated iterative recovery that reconstructed **92% of a 32-token string** from model internals. This is not a future threat; the math already works.

- **Primary leak:** vector/embedding weakness (OWASP LLM08) — and note this is *not* just LLM02 "sensitive disclosure." The exposure is structural to the token representation, not to what a human would call "PII."
- **Secondary leak:** token-inflation denial-of-service (LLM10) — homoglyphs and weird unicode blow up token counts and cost.
- **Control pattern:** never expose raw log-probs or hidden states to untrusted callers, rate-limit token endpoints, pre-scrub PII before any third-party tokenizer, and SBOM-scan the tokenizer library itself (LLM03).

### 3. Semantic Search

The received wisdom is that embeddings are "unrecognizable" vectors. The research disagrees. Embedding-inversion attacks reconstruct plaintext from stored vectors, and one production-like evaluation (Cyborg) reported **99.38% content reconstruction in under five minutes**. Your vector database is a sensitive store that almost nobody classifies as one.

- **Primary leak:** LLM08 (vector/embedding weakness) plus LLM02 — stored embeddings *and* query embeddings both leak, the latter revealing searcher intent.
- **Secondary leaks:** cross-tenant vector access (LLM06), index poisoning (LLM04).
...
RAG is where most enterprise LLM privacy failures actually happen, because the failure mode is faithful, helpful, and catastrophic: an over-broad retriever returns documents the user isn't authorized to see, and the model diligently includes the PII in its answer. Add indirect prompt injection embedded in a retrieved document — the single most-exploited RAG failure (LLM01) — and you have data exfiltration wrapped in a helpful citation.
...
- **Model/vendor inventory (LLM03):** know every provider, version, and data flow. You can't control what you haven't catalogued.
- **SBOM & dependency scan (LLM03):** CVE-scan libs, tokenizer deps, RAG loaders.
- **Training/pretrain PII scan (LLM03, LLM04):** detect PII or poisoned data in fine-tunes and corpora *before* it enters the model.
- **Provider config & key hygiene (LLM03, LLM07):** signed DPAs, no hard-coded keys, tenant isolation, and a kill-switch to cut a compromised provider.
- **License/provenance check (LLM03):** model and dataset license compliance and source attestation.
...
- **Embedding-exposure detector (LLM08):** flags vector stores or embedding logs that leaked to an untrusted party.
- **Anomaly & leak detection (LLM10, LLM02):** unusual volume, token-inflation patterns, secret regexes in outputs.
- **Drift & feedback loop (LLM09):** quality, bias, behavior-change monitoring plus user-flagged leaks feeding back into policy.
- **Audit → SIEM alerting (LLM05, LLM02):** immutable traces fan out to SOC/IR so a leak becomes an *incident*, not a footnote.
...
This maps to the current OWASP list (published 2026-08-03; the 2023 edition is legacy and renumbered). Every category is covered by at least one surface:
...
1. **Stop drawing one "Guardrails" box.** Model the surfaces separately. If your diagram can't name the tokenizer as an exposure point, it's incomplete.
...
The diagrams that accompany this article (C4 Context, Container, and the data-exposure Component view) are on GitHub under `llm-c4-controls`. The lesson they encode is the same one a flat "guardrails" diagram hides: private data in AI doesn't leak in one place. It leaks in the analyzer, the tokenizer, the vector store, the retriever, the vendor, and — last and most often — in the silence after the request completes.
